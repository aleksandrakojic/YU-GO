...
import { Stack, Button, HStack, Input, InputGroup, InputLeftAddon, VStack, Divider } from '@chakra-ui/react';
    ...
    <VStack spacing={6} alignItems="stretch" divider={<Divider />}>
      <Stack spacing={3}>
        <HStack>
          <InputGroup>
            ...
            <Input type="number" value={block} onChange={(e) => setBlock(+e.target.value)} />
          </InputGroup>{' '}
          <Button onClick={() => getBlockQuery.fetch({ params: { block_number_or_hash: '1000' } })}>getBlock</Button>
        </HStack>
      ...
      <Stack spacing={3}>
        <HStack>
          ...
          <Button onClick={() => getNativeBalanceQuery.fetch()}>getNativeBalance</Button>
        </HStack>

...
import { Autocomplete, Divider, Grid, Input } from '@mui/material';
...
export default function AddContestModal({ isOpen, onClose, onSubmit, thematics, countries }: Props) {
  ...
  const handleChange = ({ target: { value, name } }) => setContest({ ...contest, [name]: value });
  const handleClose = () => {
  ...
  return (
    <Grid container sx={{ padding: 2 }}>
      <Dialog open={isOpen} onClose={handleClose} sx={{ '& > div > div': { padding: 2, backgroundColor: '#393264' } }}>
        <DialogTitle sx={{ fontSize: '1rem', fontWeight: 'bold' }}>ADD NEW CONTEST</DialogTitle>
        <Divider />
        <DialogContent
          sx={{ display: 'flex', flexDirection: 'column', minHeight: '450px', justifyContent: 'space-between' }}
        >
          <DialogContentText>
            Create new contest and add thematics and countries for which contest will be visible
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
          ...
          />
          <Box sx={{ width: '100%', display: 'flex', justifyContent: 'space-between', padding: '20px 0px' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              <p>Voting End Date</p>
              <TextField name="votingEndDate" type="date" value={contest.votingEndDate} onChange={handleChange} />
            </Box>
          </Box>
        </DialogContent>

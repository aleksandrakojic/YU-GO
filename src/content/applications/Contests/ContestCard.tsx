...
export default function ContestCard({ contest, isCreator, index }: Props) {
	...
	const contestThematics = thematics.filter((t) => contest?.thematics?.includes(t.id));
	const contestCountries = countries.filter((c) => contest?.countries?.includes(c.id));
	...
	return (
		<Card sx={{ width: '90%', margin: '10px' }}>
					...
					<IconButton aria-label="settings">
						{contestCountries?.map((c) => (
							<FlagWrapper key={c.code}>
								<Flag code={c.code} style={{ width: '30px', marginRight: '5px' }} />
							</FlagWrapper>
						))}
						<MoreVertIcon />
					</IconButton>
			...
			<CardMedia component="img" height="280" image={renderImg()} alt="contest" />
			<CardActionsWrapper>
				<div>
					<p>
						Application End Date: <i>{contest?.applicationEndDate}</i>
					</p>
					<p>
						Voting End Date: <i>{contest?.votingEndDate}</i>
					</p>
				</div>
				<Typography variant="h4">
					Available Funds <Chip label={`${contest.availableFunds} ETH`} color="primary" />
				</Typography>
			</CardActionsWrapper>
			<CardContent>
